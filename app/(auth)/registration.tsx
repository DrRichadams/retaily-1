...
export default function RegistrationScreen() {
  ...
  // High-fidelity adaptive colors
  const activeBrandColor = isMerchant ? colors.accent : "#10B981"; // Vibrant Blue for Merchant vs Emerald for Staff

  // Dynamic Background Gradients Mapping
  const gradientColors = isDarkMode
    ? ["#0F172A", "#1E293B", "#0F172A"] 
    : ["#F8FAFC", "#E2E8F0", "#CBD5E1"]; // Polished light mode contrast canvas

  // Reanimated Shared Value for the directional connection line path
  const lineProgress = useSharedValue(0);

  useEffect(() => {
    // Map width progress based on step depth location
  ...
  const isStaffValid = useMemo(() => staffName.trim() !== "" && staffDOB.trim() !== "" && staffNationalID.trim() !== "" && staffAddress.trim() !== "", [staffName, staffDOB, staffNationalID, staffAddress]);
          ...
          <View style={styles.outerLayoutStructure}>
            ...
            <ScrollView contentContainerStyle={styles.scrollGrowContainer} bounces={false} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              <View style={styles.topAndFormFlowContainer}>
                ...
                {/* THE PROGRESS INDICATOR TRACK WINDOW */}
                <View style={styles.progressIndicatorTrack}>
                  <View style={styles.stepNodeBlock}>
                    {currentStep > 1 ? (
                      <View style={[styles.stepCircle, styles.stepCircleDone, { backgroundColor: isDarkMode ? "rgba(255,255,255,0.15)" : "rgba(11,34,64,0.12)" }]}>
                        <Ionicons name="checkmark" size={13} color={isDarkMode ? colors.secondaryText : "#475569"} />
                      </View>
                    ) : (
                      <ActivePulseCircle colors={{ ...colors, accent: activeBrandColor }} num={1} />
                    )}
                    <Text style={[styles.stepNodeLabel, { color: currentStep === 1 ? activeBrandColor : colors.secondaryText, fontWeight: currentStep === 1 ? "700" : "500" }]}>
                      {isMerchant ? "Core Setup" : "Identity"}
                    </Text>
                  </View>

                  {/* Animated Line Track Core Container */}
                  <View style={[styles.lineContainerContainer, { backgroundColor: isDarkMode ? "#334155" : "#E2E8F0" }]}>
                    <Animated.View style={[styles.animatedProgressBar, { backgroundColor: activeBrandColor }, animatedLineStyle]} />
                  </View>
                  ...
                  <View style={[styles.stepNodeBlock, !isMerchant && { opacity: 0.25 }]}>
                    {currentStep === 2 && isMerchant ? (
                      ...
                      <View style={[styles.stepCircle, { backgroundColor: isDarkMode ? colors.inputBg : "rgba(15,23,42,0.05)", borderColor: isDarkMode ? colors.inputBorder : "#CBD5E1" }]}>
                        <Text style={[styles.stepNumberText, { color: colors.secondaryText }]}>2</Text>
                      </View>
                    )}
                    <Text style={[styles.stepNodeLabel, { color: currentStep === 2 ? activeBrandColor : colors.secondaryText, fontWeight: currentStep === 2 ? "700" : "500" }]}>
                      {isMerchant ? "Localization" : "Finalize"}
                    </Text>
                  </View>
                </View>

                {/* FORM HEADER METRICS (Text wrap height issues solved) */}
                <Animated.View layout={LinearTransition} style={styles.textSection}>
                  <Text style={[styles.title, { color: colors.primaryText }]}>
                    {isMerchant ? (currentStep === 1 ? "Core Business Parameters" : "Localization & Outlets") : "Complete Staff Credentials"}
                  </Text>
                  <Text style={[styles.subtitle, { color: colors.secondaryText }]}>
                    {isMerchant 
                      ? (currentStep === 1 ? "Define your business workspace container name, unique industrial niche category, and preferred functional base operating currency setup." : "Specify your primary cloud backend physical office address and approximate initial store branch infrastructure count configurations.")
                      : "Provide your authentic field credentials and national identity profiles to securely route access validation requests directly to your regional shop administrator nodes."}
                  </Text>
                </Animated.View>

                {/* MAIN CONTENT SPACE MAPPINGS */}
                <View style={styles.mainContentArea}>
                  {isMerchant ? (
                    <View style={styles.fullWidth}>
                      {currentStep === 1 ? (
                        <Animated.View entering={FadeInRight.duration(250)} exiting={FadeOutLeft.duration(200)} style={styles.fullWidth}>
                          ...
                          <TouchableOpacity 
                            style={[styles.dropdownSelector, { backgroundColor: isDarkMode ? colors.inputBg : "#FFFFFF", borderColor: isDarkMode ? colors.inputBorder : "#CBD5E1" }]} 
                            onPress={() => { setShowCategoryDropdown(!showCategoryDropdown); setShowCurrencyDropdown(false); }}
                            activeOpacity={0.8}
                          >
                            <Text style={[styles.dropdownValue, { color: businessCategory ? colors.primaryText : "#64748B" }]}>
                              {businessCategory || "Select Business Category Niche"}
                            </Text>
                            <Ionicons name={showCategoryDropdown ? "chevron-up" : "chevron-down"} size={16} color={colors.secondaryText} />
                          </TouchableOpacity>
                          {showCategoryDropdown && (
                            <View style={[styles.dropdownMenu, { backgroundColor: isDarkMode ? colors.cardBg : "#FFFFFF", borderColor: isDarkMode ? colors.inputBorder : "#CBD5E1" }]}>
                              {BUSINESS_CATEGORIES.map((cat) => (
                                <TouchableOpacity key={cat} style={[styles.dropdownItem, { borderBottomColor: isDarkMode ? "#334155" : "#F1F5F9" }]} onPress={() => { setBusinessCategory(cat); setShowCategoryDropdown(false); }}>
                          ...
                          <TouchableOpacity 
                            style={[styles.dropdownSelector, { backgroundColor: isDarkMode ? colors.inputBg : "#FFFFFF", borderColor: isDarkMode ? colors.inputBorder : "#CBD5E1", marginTop: 16 }]} 
                            onPress={() => { setShowCurrencyDropdown(!showCurrencyDropdown); setShowCategoryDropdown(false); }}
                            activeOpacity={0.8}
                          >
                            <Text style={[styles.dropdownValue, { color: currency ? colors.primaryText : "#64748B" }]}>
                              {currency || "Select Default Operating Currency"}
                            </Text>
                            <Ionicons name={showCurrencyDropdown ? "chevron-up" : "chevron-down"} size={16} color={colors.secondaryText} />
                          </TouchableOpacity>
                          {showCurrencyDropdown && (
                            <View style={[styles.dropdownMenu, { backgroundColor: isDarkMode ? colors.cardBg : "#FFFFFF", borderColor: isDarkMode ? colors.inputBorder : "#CBD5E1" }]}>
                              {PREFERRED_CURRENCIES.map((cur) => (
                                <TouchableOpacity key={cur} style={[styles.dropdownItem, { borderBottomColor: isDarkMode ? "#334155" : "#F1F5F9" }]} onPress={() => { setCurrency(cur); setShowCurrencyDropdown(false); }}>
                                  <Text style={[styles.dropdownItemText, { color: colors.primaryText }]}>{cur}</Text>
                                </TouchableOpacity>
                              ))}
                            </View>
                          )}
                    ...
                    <Animated.View entering={FadeInRight.duration(250)} style={styles.fullWidth}>
                      <CustomInput placeholder="First Name & Surname" value={staffName} onChangeText={setStaffName} placeholderTextColor={isDarkMode ? undefined : "#64748B"} style={!isDarkMode && styles.lightInputOverride} />
                      <CustomInput placeholder="Date of Birth (DD/MM/YYYY)" value={staffDOB} onChangeText={setStaffDOB} placeholderTextColor={isDarkMode ? undefined : "#64748B"} style={!isDarkMode && styles.lightInputOverride} />
                      <CustomInput placeholder="National ID Identification Number" value={staffNationalID} onChangeText={setStaffNationalID} autoCapitalize="characters" placeholderTextColor={isDarkMode ? undefined : "#64748B"} style={!isDarkMode && styles.lightInputOverride} />
                      <CustomInput placeholder="Residential Physical Address" value={staffAddress} onChangeText={setStaffAddress} multiline placeholderTextColor={isDarkMode ? undefined : "#64748B"} style={[styles.textareaStyle, !isDarkMode && styles.lightInputOverride]} />
                    </Animated.View>
                  )}

                  <View style={styles.buttonWrapper}>
                    <CustomButton 
                      title={isMerchant ? (currentStep === 1 ? "Continue Setup" : "Initialize Business Space") : "Create Employee Profile"} 
            ...
            <View style={[styles.footerContainer, { borderTopColor: isDarkMode ? "rgba(255,255,255,0.04)" : "rgba(15,23,42,0.06)" }]}>
              <MaterialCommunityIcons name="server-security" size={13} color={colors.secondaryText} style={{ marginRight: 6 }} />
              <Text style={[styles.secureText, { color: colors.secondaryText }]}>
                Secured Cloud Space Database Initialization Node • Retaily OS
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  // High-Fidelity Step Progress Indicator Layout Styles
  ...
  // Isolated Animated Width Progress Bar Channels
  ...
  // Typography layout settings wrapped safely with zero static height cutoffs
